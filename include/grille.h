...
/**
	* \struct grille
	* \brief Structure grille: Contient le nombre de lignes et le nombre de colonnes de la grille ainsi qu'un "tableau de tableau de cellules" représentant la grille
...
/**
	...
	* \param fichier_grille - Pointeur vers le nom du fichier correspondant à la grille
	* \param g - Pointeur vers une structure grille
*/
int init_grille_from_file (char * fichier_grille, grille* g);

/**
	* \brief Alloue un espace mémoire pocycliqueur une grille de taille l*c, et initialise toutes les cellules à 0
...
/**
	...
	* \param g - Pointeur sur la grille à tester
	* \param compte_voisins_vivants - Pointeur sur la fonction qui compte les voisins vivants
	* \param vieillissement_bool - Indique l'état du mode vieillissement (1 si activé, 0 sinon)
*/
int grille_oscillante(grille *g, int (*compte_voisins_vivants) (int, int, grille), int vieillissement_bool);
...
/**
  ...
  * \param g - La grille
*/
static inline void set_temporaire_morte(int i, int j, grille g) {g.cellules[i][j] = 11;}
...
/**
  ...
  * \param g - La grille
*/
static inline void set_temporaire_vivante(int i, int j, grille g) {g.cellules[i][j] = 9;}
