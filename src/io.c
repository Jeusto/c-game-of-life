...
void debut_jeu(grille *g, grille *gc){
	...
	while (c != 'q') {
		switch (c) {
			case 'o': {
				passer_evolution = 1;
				oscillation = grille_oscillante(g, voisins_vivants, vieillissement_bool);
...
cairo_surface_t *cairo_creer_surface(int x, int y) {
  ...
  XStoreName(dpy, win, "Jeu de la vie 5.0.0");
  ...
  if (classHint) {
    ...
    surface = cairo_xlib_surface_create(dpy, win, DefaultVisual(dpy, screen), x, y);
...
void affiche_ligne (int c, int* ligne, int vieillissement_bool, int hauteur, float taille_ligne) {
	int i;
	...
	for (i=0; i<c; ++i) {
		if (ligne[i] == 0) {
			cairo_line_to(cr, MG_GAUCHE + i * taille_colonne, MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_move_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne));
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
		} else if (ligne[i] == -1) {
			cairo_line_to(cr, MG_GAUCHE + i * taille_colonne, MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + 1);
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_line_to(cr, MG_GAUCHE + i * taille_colonne, MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_move_to(cr, MG_GAUCHE + ((i) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne));
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne));
		} else {
			cairo_line_to(cr, MG_GAUCHE + i * taille_colonne, MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_move_to(cr, MG_GAUCHE + ((i-1) * taille_colonne) + 2, MG_HAUTE + (hauteur * taille_colonne) + 2);
			cairo_move_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne));
			cairo_line_to(cr, MG_GAUCHE + ((i+1) * taille_colonne), MG_HAUTE + (hauteur * taille_colonne) + taille_colonne);
			cairo_rectangle(cairo_cellules, MG_GAUCHE + i * taille_colonne + 3, MG_HAUTE + (hauteur * taille_colonne + 3),	taille_colonne - 6, taille_colonne - 6);
			cairo_fill(cairo_cellules);

			if (vieillissement_bool) {
				sprintf(age_cellule, "%d", ligne[i]);
				cairo_move_to(cr, MG_GAUCHE + ((i) * taille_colonne + (taille_colonne*0.35)),
...
void affiche_trait (int c, int hauteur, float taille_ligne) {
	...
	int i;
	for (i = 0; i < c; ++i) {
	  cairo_line_to(cr, MG_GAUCHE + LARGEUR_GRILLE, MG_HAUTE + (hauteur * taille_colonne));
	  cairo_move_to(cr, MG_GAUCHE, MG_HAUTE + (hauteur * taille_colonne));
	}
	cairo_stroke(cr);
	cairo_destroy(cr);
...
void affiche_grille (grille g, int temps_evolution, int cyclique_bool, int vieillissement_bool, int temps_oscillation){
	int i, l=g.nbl, c=g.nbc;
	char str_temps_evolution[255], str_mode_cyclique[255], str_mode_vieillissement[255], str_temps_oscillation[255];
	sprintf(str_temps_evolution, "► Temps d'évolution : %d", temps_evolution);
	sprintf(str_mode_cyclique, cyclique_bool ? "► Mode cyclique : Activé" : "► Mode cyclique : Désactivé");
	sprintf(str_mode_vieillissement, vieillissement_bool ? "► Vieillissement : Activé" : "► Vieillissement : Désactivé");
	...
	if (temps_oscillation == -1) {
	    ...
	    sprintf(str_temps_oscillation, "► Oscillation : %d pas/période", temps_oscillation);
	}
	...
	cairo_show_text(cr, "Jeu de la vie 5.0.0");
	...
	for (i=0; i<l; ++i) {
	    affiche_ligne(c, g.cellules[i], vieillissement_bool, hauteur, taille_colonne);
	    affiche_trait(c, ++hauteur, taille_colonne);
	}
	return;
}
...
void debut_jeu(grille *g, grille *gc) {
	...
	while(1) {
		...
		if (e.type==Expose && e.xexpose.count<1) {
		    affiche_grille(*g, temps_evolution, cyclique_bool, vieillissement_bool, temps_oscillation);
		} else if (e.type == KeyPress) { // Touche pressée
			if (e.xkey.keycode == 36 || e.xkey.keycode == 104) { // Touche entrée
			    evolue(g,gc,&temps_evolution,compte_voisins_vivants,vieillissement_bool);
			    refresh_grille = 1;
				...
				do {
					...
					while (1) {
					    XNextEvent(cairo_xlib_surface_get_display(surface), &e);
					    if (e.type == KeyPress && XLookupString(&e.xkey,input_buffer,255,&key,0)==1) {
					...
					if (!exit_grid_change) {
						...
						if (erreur) {
						    printf("Fichier de grille introuvable. Essayez à nouveau\n");
						    cairo_affiche_texte("", "Erreur : Fichier de grille introuvable");
						    }
						    fclose(fichier_grille_test);
						    fichier_grille_test = NULL;
						} else {
						    erreur = 1;
						    cairo_affiche_texte("", "Fichier de grille introuvable. Essayez à nouveau ");
						}
					}
			    ...
			    temps_oscillation = grille_oscillante(g, compte_voisins_vivants, vieillissement_bool);
			    refresh_grille = 1;
