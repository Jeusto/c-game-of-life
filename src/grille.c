/**
  * \file grille.c
  * \brief Gestion des grilles dont l'allocation, la libération de mémoire et charger une grille depuis un fichier
  * \author Saday Arhun
*/
...
int grilles_egales(grille *g1, grille *g2) {
	...
	for (i = 0; i < g1->nbl; i++) {
		for (j = 0; j < g1->nbc; j++) {
			if ((!est_vivante(i,j,*g1) && est_vivante(i,j,*g2)) || (est_vivante(i,j,*g1) && !est_vivante(i,j,*g2))) {
				return 0;
			}
		}
	}
	...
	int i = 0;
	 ...
	 do {
		while (temps_evolution_oscillation < max_interval) {
			evolue(&grille_copiee_modifiee,&tmp,&temps_evolution_oscillation,compte_voisins_vivants,vieillissement_bool);
		...
		evolue(&grille_copiee,&tmp,&temps_evolution_oscillation,compte_voisins_vivants,vieillissement_bool);
